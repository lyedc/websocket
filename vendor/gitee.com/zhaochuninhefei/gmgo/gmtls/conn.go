...
func (hc *halfConn) incSeq() {
	...
	panic("gmtls: sequence number wraparound")
}
...
func (c *Conn) readRecordOrCCS(expectChangeCipherSpec bool) error {
	...
	if c.input.Len() != 0 {
		return c.in.setErrorLocked(errors.New("gmtls: internal error: attempted to read record with pending application data"))
	}
	c.input.Reset(nil)
...
var (
	errShutdown = errors.New("gmtls: protocol is shutdown")
)
...
func (c *Conn) handlePostHandshakeMessage() error {
	...
	case *newSessionTicketMsgTLS13:
		zclog.Debug("===== 客户端接收到 newSessionTicketMsgTLS13")
...
func (c *Conn) Close() error {
	...
	if c.handshakeComplete() {
		if err := c.closeNotify(); err != nil {
			alertErr = fmt.Errorf("gmtls: failed to send closeNotify alert (but connection was closed anyway): %w", err)
		}
	}
...
var errEarlyCloseWrite = errors.New("gmtls: CloseWrite called before handshake complete")
...
func (c *Conn) handshakeContext(ctx context.Context) (ret error) {
	...
	if c.handshakeErr == nil && !c.handshakeComplete() {
		c.handshakeErr = errors.New("gmtls: internal error: handshake should have had a result")
	}
